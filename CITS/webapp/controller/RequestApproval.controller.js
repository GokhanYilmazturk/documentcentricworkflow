sap.ui.define([
    ...
    Filter, FilterOperator, FileUploaderParameter, UploadCollectionParameter, DateFormat, FilterBar, FilterGroupItem, Input, models) {
    "use strict";

    var tempFolderObjId,
        token;

    return BaseController.extend("com.sap.bpm.CITS.controller.ExpenditureRequest", {
        /**
         * Called when a controller is instantiated and its View controls (if available) are already created.
         * Can be used to modify the View before it is displayed, to bind event handlers and do other one-time initialization.
         * @memberOf com.sap.bpm.CITS.view.ExpenditureRequest
         */
        onInit: function () {
            ...
            //gokhan
            this.setModel(models.createUsersModel(), "ooUser");
        ...
        */
        ...
        getUserData: function (sEmail) {
            ...
            //gokhan
            //var filter = new Filter("emails", FilterOperator.eq, sEmail);
            var userData = oThisController.getView().getModel("ooUser").getData();
            oThisController._presetRequesterFields(userData.Users[0]);

            var oView = oThisController.getView();
            oView.setBusy(true);

            var sUrl = '/comsapbpmCITS/scim/service/scim/Users?filter=emails eq "' + sEmail + '"';
            var oSettings = {
                "url": sUrl,
                "method": "GET"
            };
        ...
        */
        ...
        checkIfFolderExists: function (folderName) {
            ...
            var oMdlCommon = this.getParentModel("mCommon");
            var sCustomer = oMdlCommon.getProperty("/sCustomer");
            var oUploadCollection = this.byId("UploadCollection");
            oUploadCollection.setBusy(true);
            var responseStatusCode;

            if (folderName == "WorkflowManagement") {
                var sUrl = "/comsapbpmCITS/docservice/WorkflowManagement/";
            } else if (folderName == "DocumentCentricApprovalProcess") {
                var sUrl = "/comsapbpmCITS/docservice/WorkflowManagement/DocumentCentricApprovalProcess/";
            } else if (folderName == sCustomer)  {
                var sUrl = "/comsapbpmCITS/docservice/WorkflowManagement/DocumentCentricApprovalProcess/" + folderName + "/";
            } else {
                var sUrl = "/comsapbpmCITS/docservice/WorkflowManagement/DocumentCentricApprovalProcess/" + folderName + "/";
            }
            var oSettings = {
            ...
            if (folderName == "WorkflowManagement") {
                ...
                if (responseStatusCode == 404) {
                    this.createCustomerFolder(folderName);
                } else if (responseStatusCode == 200) {
                    console.log("folder with a name '" + sCustomer + "' already exisits");
                } else {
                    console.log("something is wrong");
                }
            } else {
        ...
        createFolder: function (folderName) {
            ...
            if (folderName == "WorkflowManagement") {
                ...
                var sUrl = "/comsapbpmCITS/docservice/";
            } else if (folderName == "DocumentCentricApprovalProcess") {
                ...
                var sAttachmentsUploadURL = "/comsapbpmCITS/docservice/WorkflowManagement/DocumentCentricApprovalProcess/" + folderName + "/";
        ...
        createCustomerFolder: function (targetFolderName) {
            console.log("creating a customer folder 'WorkflowManagement/DocumentCentricApprovalProcess/" + targetFolderName + "/'");
            ...
            $.ajax(oSettings)
                .done(function (results) {
                    ...
                    var oUploadCollection = oThisController.byId("UploadCollection");
                    var sAttachmentsUploadURL = "/comsapbpmCITS/docservice/WorkflowManagement/DocumentCentricApprovalProcess/" + targetFolderName + "/";
                    oUploadCollection.setUploadUrl(sAttachmentsUploadURL);
        ...
        // create permanent folder
        createTargetFolder: function (customerFolderName, targetFolderName) {
            console.log("creating a permanent folder 'WorkflowManagement/DocumentCentricApprovalProcess/" + customerFolderName + "/" + targetFolderName + "/'");

            var sUrl = "/comsapbpmCITS/docservice/WorkflowManagement/DocumentCentricApprovalProcess/" + customerFolderName + "/";
            ...
            $.ajax(oSettings)
                .done(function (results) {
                    var targetFolderId = results.succinctProperties["cmis:objectId"];
                    var oUploadCollection = oThisController.byId("UploadCollection");
                    var sAttachmentsUploadURL = "/comsapbpmCITS/docservice/WorkflowManagement/DocumentCentricApprovalProcess/"  + customerFolderName + "/" + targetFolderName + "/";
                    oUploadCollection.setUploadUrl(sAttachmentsUploadURL);
                    oThisController.oAttachmentsModel.refresh(true);
        ...
        */
        ...
        getUsers: function (oColumns, sInputField) {
            ...
            var userData = oThisController.getView().getModel("ooUser").getData();
            oView.setBusy(false);
            oThisController._mapUsers(oColumns, sInputField, userData.Users, 0, 0);
            
            

            var sUrl = "/comsapbpmCITS/scim/service/scim/Users/";
            var oSettings = {
                "url": sUrl,
                "method": "GET"
            };
        ...
        getUsersAdd: function (iteration, oColumns, sInputField) {
            ...
            var sUrl = "/comsapbpmCITS/scim/service/scim/Users?startIndex=" + startIndex;
            var oSettings = {
                "url": sUrl,
                "method": "GET",
                "async": false
        ...
        fnCreateFragment: function (oColumns, oInputData) {
            ...
            if (!this._oValueHelpDialog) {
                this._oValueHelpDialog = sap.ui.xmlfragment("com.sap.bpm.CITS.view.BusinessValueHelp", this);
        ...
        */
        ...
        fetchToken: function () {
            ...
            oThisController.getView().setBusy(true);
            $.ajax({
                url: "/comsapbpmCITS/workflowruntime/v1/xsrf-token",
                method: "GET",
                headers: {
                    "X-CSRF-Token": "Fetch"
                },
        ...
        startInstance: function (workflowtoken) {
            ...
            var oThisController = this,
                oMdlCommon = oThisController.getParentModel("mCommon"),
                oAttachmentsModel = oThisController.getModel();
            var sUrl = "/comsapbpmCITS/workflowruntime/v1/workflow-instances";
            ...
            var sPayload = {
                ...
                "context": {
                    ...
                    "Customer": oMdlCommon.getProperty("/sCustomer"),
                    "ApprovalSteps": aApprovalSteps,
                    "Attachments": aAttachments,
                    "path": {
                        "to": {
                            "comment": oMdlCommon.getProperty("/oRequesterDetails/sRequesterComment")
                        }
                    }
                }
            };
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    var customer = oMdlCommon.getProperty("/sCustomer");
                    oThisController.checkIfFolderExists(customer);
                    //oThisController.createCustomerFolder(customer);

                    oThisController.createTargetFolder(customer, workflowId);
